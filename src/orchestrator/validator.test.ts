...
describe('Validator - New Primitives', () => {
  ...
  describe('Turn Ownership - Orchestrator Authority', () => {
    ...
    it('validates nested player paths for turn ownership', () => {
      mockState.players.p1.inventory = { gold: 10 }
  ...
  describe('Decision Points - Orchestrator Enforcement', () => {
    beforeEach(() => {
      ...
      mockState.players.p1.pathChoice = null
    })
    ...
    it('allows position changes after decision is made', () => {
      mockState.players.p1.pathChoice = 'A'
      const actions = [{action: 'SET_STATE', path: 'players.p1.position', value: 10}]
      const result = validateActions(actions, mockState, mockStateManager)
      expect(result.valid).toBe(true)
    })
    ...
    it('validates correct requiredField (field-specific)', () => {
      mockState.players.p1.pathChoice = 'A'
      mockState.decisionPoints[0].requiredField = 'otherChoice'
      mockState.players.p1.otherChoice = null
    ...
    it('allows moves when no decision point at position', () => {
      mockState.players.p1.position = 3
  ...
  describe('Path Validation', () => {
    ...
    it('validates deeply nested existing paths', () => {
      mockState.players.p1.inventory = { items: { sword: { damage: 10 } } }
  ...
  describe('Context-Aware Validation - Square Effect Processing', () => {
    it('blocks PLAYER_ROLLED when orchestrator is processing square effect', () => {
      ...
      const result = validateActions(actions, mockState, mockStateManager, mockOrchestrator)
  ...
  describe('Edge Cases', () => {
    ...
    it('rejects non-array input', () => {
      ...
      const result = validateActions(actions as unknown as unknown[], mockState, mockStateManager)
      expect(result.valid).toBe(false)
