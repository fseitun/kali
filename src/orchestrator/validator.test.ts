...
describe('Validator - New Primitives', () => {
  ...
  describe('PLAYER_ROLLED', () => {
    it('validates with positive value', () => {
      ...
      const result = validateActions(actions, mockState, mockStateManager as unknown as StateManager)
  ...
  describe('Turn Ownership - Orchestrator Authority', () => {
    ...
    it('validates nested player paths for turn ownership', () => {
      (mockState.players as Record<string, unknown>).p1 = { ...((mockState.players as Record<string, unknown>).p1 as object), inventory: { gold: 10 } }
      const actions = [{action: 'SET_STATE', path: 'players.p1.inventory', value: {gold: 20}}]
      const result = validateActions(actions, mockState, mockStateManager as unknown as StateManager)
      expect(result.valid).toBe(true)
    })
  })
  ...
  describe('Decision Points - Orchestrator Enforcement', () => {
    ...
    it('validates correct requiredField (field-specific)', () => {
      (mockState.players as Record<string, Record<string, unknown>>).p1.pathChoice = 'A';
      (mockState.decisionPoints as Record<string, unknown>[])[0].requiredField = 'otherChoice';
  ...
  describe('Path Validation', () => {
    ...
    it('validates deeply nested existing paths', () => {
      (mockState.players as Record<string, Record<string, unknown>>).p1.inventory = { items: { sword: { damage: 10 } } }
  ...
  describe('Context-Aware Validation - Square Effect Processing', () => {
    it('blocks PLAYER_ROLLED when orchestrator is processing square effect', () => {
      ...
      const result = validateActions(actions, mockState, mockStateManager as unknown as StateManager, mockOrchestrator)
  ...
  describe('Edge Cases', () => {
    ...
    it('accepts actions with extra unknown fields (extensibility)', () => {
      const actions = [{action: 'NARRATE', text: 'Hi', extraField: 'ignored', anotherField: 123}]
      const result = validateActions(actions, mockState, mockStateManager as unknown as StateManager)
      expect(result.valid).toBe(true)
    })
    ...
    it('rejects non-array input', () => {
      ...
      const result = validateActions(actions as unknown as unknown[], mockState, mockStateManager as unknown as StateManager)
      expect(result.valid).toBe(false)
