...
import { GamePhase, ExecutionContext } from './types'
...
describe('DecisionPointEnforcer', () => {
  ...
  beforeEach(() => {
    ...
    decisionPointEnforcer = new DecisionPointEnforcer(stateManager, mockProcessTranscript)
  })
  ...
  describe('enforceDecisionPoints()', () => {
    ...
    it('should do nothing when no current turn set', async () => {
      ...
      stateManager.set('decisionPoints', [
        { position: 5, requiredField: 'pathChoice', prompt: 'Choose your path' }
      ])
    ...
    it('should do nothing when current player not at decision point', async () => {
      stateManager.set('decisionPoints', [
        { position: 10, requiredField: 'pathChoice', prompt: 'Choose your path' }
      ])
    ...
    it('should pass correct context depth (depth + 1)', async () => {
      ...
      await decisionPointEnforcer.enforceDecisionPoints({ depth: 2, maxDepth: 5 })
    ...
    it('should skip enforcement when approaching max depth', async () => {
      ...
      await decisionPointEnforcer.enforceDecisionPoints({ depth: 4, maxDepth: 5 })
    ...
    it('should skip enforcement when at max depth', async () => {
      ...
      await decisionPointEnforcer.enforceDecisionPoints({ depth: 5, maxDepth: 5 })
    ...
    it('should handle multiple decision points (only enforce current position)', async () => {
      stateManager.set('decisionPoints', [
        { position: 3, requiredField: 'pathChoice', prompt: 'Choose path A or B' },
        { position: 5, requiredField: 'toolChoice', prompt: 'Choose tool X or Y' },
        { position: 10, requiredField: 'directionChoice', prompt: 'Go left or right' }
      ])
