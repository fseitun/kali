  ...
  describe('Animal Encounters', () => {
    it('handles animal encounter with power check failure', async () => {
      const responses: PrimitiveAction[][] = [
        ...
        [
          { action: 'NARRATE', text: 'You encounter a Cobra! Roll for power check.' }
        ]
      ]
    ...
    it('handles animal encounter triggering square effect', async () => {
      const responses: PrimitiveAction[][] = [
        ...
        [
          { action: 'SET_STATE', path: 'players.p1.bonusDiceNextTurn', value: true },
          { action: 'SET_STATE', path: 'players.p1.points', value: 4 },
          { action: 'NARRATE', text: 'You passed the power check and riddle! +4 points and bonus dice!' }
        ]
      ]
      ...
      const initialState: GameState = {
        ...
        players: {
          p1: { id: 'p1', name: 'Alice', position: 3, bonusDiceNextTurn: false, points: 0 },
          p2: { id: 'p2', name: 'Bob', position: 0 }
        },
        board: {
          winPosition: 100,
    ...
    it('applies bonus dice after riddle success', async () => {
      const responses: PrimitiveAction[][] = [
        [
          ...
          { action: 'SET_STATE', path: 'players.p1.bonusDiceNextTurn', value: false },
  ...
  describe('Decision Points', () => {
    it('blocks movement until path choice is made', async () => {
      ...
      const initialState: GameState = {
        ...
        decisionPoints: [
          { position: 0, requiredField: 'pathChoice', prompt: 'Choose path A or B?' }
        ]
      }
    ...
    it('teleports forward via board.moves and processes square effect', async () => {
      const responses: PrimitiveAction[][] = [
        ...
        [
          ...
          { action: 'NARRATE', text: 'Portal activated! Inverse mode deactivated.' }
        ]
      ]
      ...
      const initialState: GameState = {
        ...
        board: {
          ...
          moves: { '45': 82 },
          squares: {
            '45': { type: 'portal', name: 'Portal Forward', destination: 82, inverseMode: 'deactivate' },
            '82': { type: 'portal', name: 'Portal Destination', destination: 82, inverseMode: 'deactivate' }
          }
        }
      }
    ...
    it('teleports backward and activates inverse mode via square effect', async () => {
      const responses: PrimitiveAction[][] = [
        ...
        [
          ...
          { action: 'NARRATE', text: 'Portal activated! Inverse mode activated.' }
        ]
      ]
      ...
      const initialState: GameState = {
        ...
        board: {
          ...
          moves: { '82': 45 },
          squares: {
            '82': { type: 'portal', name: 'Portal Backward', destination: 45, inverseMode: 'activate' },
            '45': { type: 'portal', name: 'Portal Destination', destination: 45, inverseMode: 'activate' }
          }
        }
      }
  ...
  describe('Complex Mechanics', () => {
    it('handles magic door with heart requirements', async () => {
      const responses: PrimitiveAction[][] = [
        ...
        [
          { action: 'NARRATE', text: 'You need to roll 5 or higher. You have 2 hearts.' }
        ]
      ]
      ...
      const initialState: GameState = {
        ...
        board: {
          ...
          moves: {},
          squares: {
            '186': { type: 'special', name: 'Magic Door', effect: 'magicDoorCheck' }
          }
        }
      }
    ...
    it('handles instrument usage in correct habitat', async () => {
      const responses: PrimitiveAction[][] = [
        ...
        [
          ...
          { action: 'NARRATE', text: 'You used the drum of the forest! The bear sleeps. +3 points.' }
        ]
      ]
      ...
      const initialState: GameState = {
        ...
        players: {
          p1: { id: 'p1', name: 'Alice', position: 57, instruments: ['drum_forest'], points: 0 }
        },
        board: {
          winPosition: 196,
          moves: {},
          squares: {
            '59': { type: 'animal', name: 'Bear', power: 3, points: 3, habitat: 'forest', instrument: 'drum_forest' }
          }
        }
      }
