...
describe("Product scenario: Normalize For Wake Word", () => {
  it("Expected outcome: Lowercases and collapses repeated letters", () => {
    expect(normalizeForWakeWord("  KALLII  ")).toBe("kali");
    expect(normalizeForWakeWord("callee")).toBe("cale");
  });
});

describe("Product scenario: Levenshtein", () => {
  it("Expected outcome: Returns 0 for equal strings", () => {
    expect(levenshtein("kali", "kali")).toBe(0);
  });
  it("Expected outcome: Returns 1 for one substitution", () => {
    expect(levenshtein("kali", "cali")).toBe(1);
  });
  it("Expected outcome: Returns correct distance for longer word", () => {
    expect(levenshtein("kali", "callie")).toBe(3);
  });
});

describe("Product scenario: Is Wake Word Match", () => {
  it("Expected outcome: Matches exact substring", () => {
  ...
  it("Expected outcome: Matches normalized stutter", () => {
    expect(isWakeWordMatch("kallli", VARIANTS)).toBe(true);
    expect(isWakeWordMatch("caalli", VARIANTS)).toBe(true);
  });

  it("Expected outcome: Matches common ASR misrecognitions as words", () => {
  ...
  it("Expected outcome: Matches within edit distance 1", () => {
  ...
  it("Expected outcome: Returns false for empty or whitespace", () => {
