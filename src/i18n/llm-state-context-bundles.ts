...
export const llmStateContextEsAR: LlmStateContextBundle = {
  ...
  riddlePhaseNoStructured:
    '⚠️ RIDDLE ({playerName}) phase=riddle.{antiLeak}{encounterHints}{narrationShape} Pedí una adivinanza con exactamente CUATRO opciones. Tiene que ser del reino animal (animales, hábitats, comportamiento, alimentación, clasificación). Devolvé ASK_RIDDLE con "text", "options" (4 strings), "correctOption" (texto exacto de la opción correcta), opcional "correctOptionSynonyms". Después NARRATE con la adivinanza y opciones. Cuando responda el usuario, PLAYER_ANSWERED con lo que dijo — no uses RIDDLE_RESOLVED.{helpInst} [current]',
  riddlePhaseStructuredPrefix:
    "⚠️ RIDDLE ({playerName}) phase=riddle. El usuario debe elegir una de las cuatro opciones. PLAYER_ANSWERED con lo que dijo — no uses RIDDLE_RESOLVED. El orquestador resuelve acierto/error (match estricto y después LLM).",
  riddleCurrentOptions: " Opciones actuales: {optionsList}. PLAYER_ANSWERED con la respuesta del usuario (texto de opción o lo que dijo).",
  powerCheckBlock:
  ...
  powerCheckHelpManyDice: ' Si pregunta qué hacer → NARRATE «Tirá {n} dados... decime el resultado.»',
  powerRollOneDie:
  ...
  directionalHelpManyDice: ' Si pregunta qué hacer → NARRATE «Tirá {n} dados... decime el resultado.»',
...
export const llmStateContextEnUS: LlmStateContextBundle = {
  ...
  riddlePhaseStructuredPrefix:
    "⚠️ RIDDLE ({playerName}) phase=riddle. The user must choose one of the four options. Return PLAYER_ANSWERED with what the user said — do NOT use RIDDLE_RESOLVED. The orchestrator resolves correct/incorrect (strict match then LLM).",
