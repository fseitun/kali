...
describe('i18n', () => {
  ...
  describe('t (translation function)', () => {
    ...
    it('should interpolate parameters', () => {
      expect(t('setup.welcome', { game: 'Snakes & Ladders' }))
        .toBe("Welcome to Snakes & Ladders! Let's get started.")
    })

    it('should interpolate multiple parameters', () => {
      expect(t('setup.playerCount', { min: 2, max: 4 }))
        .toBe('How many players? The maximum is 4.')
    })

    it('should handle numeric parameters', () => {
      expect(t('game.position', { position: 15 }))
        .toBe("You're at position 15.")
    })
    ...
    it('should handle partial parameter replacement', () => {
      expect(t('setup.welcome', { game: 'Chess' }))
        .toBe("Welcome to Chess! Let's get started.")
    })

    it('should handle complex nested translations', () => {
      expect(t('ui.savedGameDetected', { wakeWord: 'Zookeeper' }))
        .toBe('Saved game detected. Say "Zookeeper, continue" or "Zookeeper, new game"')
    })
  })
  ...
  describe('getNumberWords', () => {
    it('should return English number words by default', () => {
      ...
      expect(words).toEqual(['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten'])
    })

    it('should return Spanish number words when locale set', () => {
      setLocale('es-AR')
      const words = getNumberWords()
      expect(words).toEqual(['cero', 'uno', 'dos', 'tres', 'cuatro', 'cinco', 'seis', 'siete', 'ocho', 'nueve', 'diez'])
    })
